...
#ifndef COMMUNICATION_ZMQHELPERS_H
#define COMMUNICATION_ZMQHELPERS_H
#include <zmq.h>
#include <assert.h>
#include <signal.h>
#include <stdarg.h>
...
using ZMQContext=void*;
using ZMQPublisher=void*;
using ZMQSubscriber=void*;
using ZMQSocket=void*;
...
//  is being terminated.
static char * s_recv (ZMQSocket socket) {
    enum { cap = 256 };
    char buffer [cap];
...
static int s_sendmore (ZMQSocket socket, char *string) {
...
//
static void s_dump (ZMQSocket socket)
{
    int rc;
...
static void s_set_id (ZMQSocket socket)
{
    char identity [10];
...
#endif //COMMUNICATION_ZMQHELPERS_H
